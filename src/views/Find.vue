    ...
    setup() {
        ...
        const methods = {
            searchNovels(){
                ...
                proxy.$novelrequest.get('/api/novels/search',{
                    ...
                    if(res.status == 200 && res.data.errno == 0){
                        ...
                        result.map((item) => {
                            item.novelSearchName = getSearchRed(state.searchValue,item.novelName)
                        })
            ...
            // 获取热门小说排行
            getHotNovels(){
                proxy.$novelrequest.get('/api/novels/getHotNovel',{
                    limit:10
                }).then((res) => {
                    if(res.status == 200 && res.data.errno == 0){
