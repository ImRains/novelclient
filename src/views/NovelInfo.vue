<template>
    ...
    <div class="novelCell">
        ...
        <ul class="chapterList"  @click="goTargetChapter($event)">
            <li v-for="(item,index) in chapterData.last" :key="index" :data-chapterindex="item.chapterindex">
                {{item.chaptername}}
            </li>
        </ul>
    </div>
    <div class="novelCell">
        <div class="allChapter">
            ...
            <nut-icon name="retweet" class="rightIcon" @click="reverseAllChapter"></nut-icon>
        </div>
        <nut-divider :style="{ color: '#ccc', borderColor: '#ccc' }"/>
        <ul class="chapterList" @click="goTargetChapter($event)">
    ...
    setup() {
        ...
        const methods = {
            ...
            getNoverInfo(){
                ...
                proxy.$novelrequest.get('/api/novels/getNovelInfo',{
                    title,
                    sourceUrl,
                    source
                }).then((res) => {
                    if(res.status == 200 && res.data.errno == 0){
                        ...
                        chapterData.last = JSON.parse(JSON.stringify(chapter.rows)).reverse().slice(0,5)
