<template>
    ...
    <nut-popup position="right" v-model:visible="isShowChapter" :style="{ width, height,background }" :destroy-on-close="popupConf.isDestory">
        ...
        <ul class="chapterList" @click="goTargetChapter($event)">
            <li v-for="(item,index) in chapterData.all.rows" :key="index" :data-chapterindex="item.chapterindex">
                {{item.chaptername}}
            </li>
        </ul>
    </nut-popup>
    <div class="chapterContent">
        ...
        <p class="chapterContentInfo" style="white-space: pre-wrap;" v-text="chapterMsg.msg.chaptercontent"></p>
    </div>
...
export default {
    setup() {
        ...
        const methods = {
            ...
            // 获取小说章节信息
            getNoverInfo(){
                proxy.$novelrequest.get('/api/novels/getNovelInfoById',{
            ...
            getChapterInfo(novelId,chapterindex){
                proxy.$novelrequest.get('/api/novels/chaptercontent',{
                    novelId,
                    chapterindex
                }).then((res) => {
                    if(res.status == 200 && res.data.errno == 0){
